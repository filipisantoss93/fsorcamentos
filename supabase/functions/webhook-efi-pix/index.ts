...
function extrairTxidsDoWebhook(body: any): string[] {
  ...
  if (body?.pix && Array.isArray(body.pix)) {
    for (const item of body.pix) {
      if (item?.txid) {
        txids.add(String(item.txid));
      }
    }
  }

  if (body?.txid) {
    txids.add(String(body.txid));
  }
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Webhook Efí recebido:", JSON.stringify(body));
    ...
    if (!txids.length) {
      return respostaJson({
        sucesso: true,
        mensagem: "Webhook recebido, mas nenhum txid foi encontrado.",
      });
    }
    ...
    for (const txid of txids) {
      ...
      if (pagamento.status === "pago") {
        resultados.push({
          txid,
          sucesso: true,
          mensagem: "Pagamento já estava marcado como pago.",
        });
        continue;
      }
      ...
      if (perfilError) {
        ...
        resultados.push({
          txid,
          sucesso: false,
          erro: "Pagamento marcado como pago, mas não foi possível buscar perfil.",
        });
        continue;
      }

      const dataExpiracaoAtual = perfil?.plano_expira_em
        ? new Date(perfil.plano_expira_em)
        : null;

      const baseExpiracao =
        dataExpiracaoAtual && dataExpiracaoAtual > agora
          ? dataExpiracaoAtual
          : agora;
      ...
      if (updatePerfilError) {
        console.error("Erro ao liberar plano:", updatePerfilError);
        resultados.push({
          txid,
          sucesso: false,
          erro: "Pagamento pago, mas erro ao liberar plano.",
        });
        continue;
      }
    ...
    return respostaJson({
      erro: error instanceof Error ? error.message : "Erro interno no webhook.",
    }, 500);
  }
});