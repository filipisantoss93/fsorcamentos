...
function getEnvObrigatorio(nome: string) {
  ...
  if (!valor) {
    throw new Error(`Secret ausente: ${nome}`);
  }
...
function gerarTxid() {
  ...
  /*
    TXID Pix pode ter até 35 caracteres.
    Usamos prefixo FS + timestamp + aleatório.
  */
...
function extrairCertificadoEChave(pemCompleto: string) {
  const certMatch = pemCompleto.match(
    /-----BEGIN CERTIFICATE-----[\s\S]*?-----END CERTIFICATE-----/
  );

  const keyMatch = pemCompleto.match(
    /-----BEGIN (?:RSA )?PRIVATE KEY-----[\s\S]*?-----END (?:RSA )?PRIVATE KEY-----/
  );

  if (!certMatch || !keyMatch) {
    throw new Error(
      "Não foi possível encontrar CERTIFICATE e PRIVATE KEY no PEM. Verifique se o certificado .p12 foi convertido corretamente."
    );
  }
...
function montarHttpClientEfi() {
  const certKeyBase64 = getEnvObrigatorio("EFI_CERT_KEY_PEM_BASE64");
  const pemCompleto = base64ToText(certKeyBase64);
...
function getBaseUrlEfi() {
  const env = String(Deno.env.get("EFI_ENV") || "homologation")
    .toLowerCase()
    .trim();
...
async function obterTokenEfi(client: Deno.HttpClient) {
  ...
  if (!resposta.ok) {
    console.error("Erro OAuth Efí:", texto);
    throw new Error(`Erro ao autenticar na Efí: ${texto}`);
  }
  ...
  try {
    ...
    throw new Error(`Resposta inválida da Efí ao autenticar: ${texto}`);
  }

  if (!dados.access_token) {
    throw new Error(`Token Efí não retornado: ${texto}`);
  }

  return dados.access_token as string;
}

async function criarCobrancaPixEfi(params: {
  ...
  console.log("Criando cobrança Pix Efí:", {
    ...
    chavePixFinal: chavePix.slice(-6),
  });
  ...
  if (!resposta.ok) {
    console.error("Erro criar cobrança Efí:", texto);
    throw new Error(`Erro ao criar cobrança Pix na Efí: ${texto}`);
  }

  try {
    return JSON.parse(texto);
  } catch {
    throw new Error(`Resposta inválida da Efí ao criar cobrança: ${texto}`);
  }
}

async function gerarQrCodeEfi(params: {
  client: Deno.HttpClient;
  accessToken: string;
  locId: number | string;
}) {
  ...
  if (!resposta.ok) {
    console.error("Erro gerar QR Code Efí:", texto);
    throw new Error(`Erro ao gerar QR Code Pix: ${texto}`);
  }

  try {
    return JSON.parse(texto);
  } catch {
    throw new Error(`Resposta inválida da Efí ao gerar QR Code: ${texto}`);
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    const supabaseServiceRoleKey = getEnvObrigatorio(
    ...
    if (!plano) {
      return respostaJson(
        {
          erro: "Período inválido. Use mensal, semestral ou anual.",
        },
        400
      );
    }
    ...
    if (!locId) {
      console.error("Cobrança sem loc.id:", cobranca);

      throw new Error(
        `A cobrança Pix foi criada, mas não retornou loc.id: ${JSON.stringify(
          cobranca
        )}`
      );
    }

    const qr = await gerarQrCodeEfi({
      client: efiClient,
      accessToken,
      locId,
    });
    ...
    if (insertError) {
      console.error("Erro ao salvar pagamento_pix:", insertError);

      throw new Error(
        `Pix gerado, mas não foi possível salvar no banco: ${insertError.message}`
      );
    }

    console.log("Pagamento Pix salvo no Supabase:", {
