// FS Orçamentos - runtime consolidado de orcamentos.html + PDF
(function () {
  'use strict';
  ...
  function setTexto(id, valor) { const el = $(id); if (el) el.textContent = valor; }
  ...
  function localizarOrcamento(ref) {
    if (ref && typeof ref === 'object') return ref;
    const id = String(ref || '').trim();
    if (!id) return null;
    return estado.orcamentos.find(o => String(o.id) === id) || null;
  }

  function linkPublicoOrcamento(ref) {
    const orcamento = localizarOrcamento(ref) || ref;
    ...
    const id = String(orcamento?.id || ref || '').trim();
  ...
  function injetarEstilo() {
    ...
    style.textContent = `
      body{background:linear-gradient(180deg,#ffffff 0%,#f8fafc 60%,#eef2f7 100%)!important;color:#111827!important;}
      ...
      .orcamentos-header,.filtro-flutuante,.resumo-financeiro,.orcamentos-lista-card{background:#fff!important;border:1px solid #e5e7eb!important;border-radius:10px!important;box-shadow:0 8px 22px rgba(15,23,42,.08)!important;margin-bottom:10px!important;}
      .orcamentos-header{display:flex!important;justify-content:space-between!important;align-items:center!important;gap:10px!important;padding:14px!important;border-top:4px solid #64748b!important;}
      .orcamentos-header h1,.resumo-header h2,.orcamentos-lista-topo h2{margin:0!important;color:#0f172a!important;line-height:1.15!important;}
      .orcamentos-header small,.orcamentos-lista-topo p,.resumo-header small,.card-resumo small{color:#64748b!important;font-weight:700!important;}
      .btn-novo-orcamento-topo,.btn-primario-modal,.btn-pdf-orcamento,.btn-whatsapp-orcamento{display:inline-flex!important;align-items:center!important;justify-content:center!important;min-height:36px!important;padding:8px 12px!important;border-radius:8px!important;background:#64748b!important;color:#fff!important;border:1px solid #64748b!important;text-decoration:none!important;font-size:12px!important;font-weight:950!important;cursor:pointer!important;box-shadow:none!important;}
      ...
      .filtro-grupo label{display:block!important;margin-bottom:4px!important;color:#374151!important;font-size:11px!important;font-weight:900!important;text-transform:uppercase!important;}
      .filtro-grupo input,.filtro-grupo select{width:100%!important;min-height:36px!important;border:1px solid #cbd5e1!important;border-radius:8px!important;padding:7px 9px!important;background:#fff!important;color:#111827!important;box-sizing:border-box!important;font-size:12px!important;}
      .resumo-financeiro,.orcamentos-lista-card{overflow:hidden!important;}
      .resumo-header,.orcamentos-lista-topo{display:flex!important;justify-content:space-between!important;align-items:flex-start!important;gap:10px!important;padding:12px 14px!important;background:#f8fafc!important;border-bottom:1px solid #e5e7eb!important;color:#111827!important;}
      .btn-toggle-resumo{min-height:32px!important;border-radius:8px!important;border:1px solid #cbd5e1!important;background:#fff!important;color:#111827!important;font-size:11px!important;font-weight:900!important;padding:6px 9px!important;cursor:pointer!important;}
      .resumo-conteudo{padding:10px!important;}.cards-resumo{display:grid!important;grid-template-columns:repeat(5,minmax(0,1fr))!important;gap:8px!important;}
      .card-resumo{border:1px solid #e5e7eb!important;border-radius:10px!important;padding:10px!important;background:#fff!important;min-width:0!important;box-shadow:none!important;}
      .card-resumo:nth-child(even){background:#f8fafc!important;}.card-resumo strong{display:block!important;color:#475569!important;font-size:10px!important;text-transform:uppercase!important;line-height:1.15!important;margin-bottom:5px!important;}
      .card-resumo .valor-resumo{display:block!important;color:#111827!important;font-size:18px!important;font-weight:950!important;line-height:1.1!important;word-break:break-word!important;}
      .lista-orcamentos{padding:0!important;}.tabela-wrapper{overflow-x:auto!important;}.tabela-orcamentos{width:100%!important;border-collapse:collapse!important;background:#fff!important;table-layout:fixed!important;}
      .tabela-orcamentos th,.tabela-orcamentos td{padding:8px 9px!important;border-bottom:1px solid #e5e7eb!important;font-size:12px!important;line-height:1.25!important;text-align:left!important;vertical-align:middle!important;white-space:normal!important;word-break:break-word!important;}
      .tabela-orcamentos th{background:#f1f5f9!important;color:#0f172a!important;font-size:11px!important;text-transform:uppercase!important;font-weight:950!important;}.linha-orcamento{cursor:pointer!important;}.linha-orcamento:nth-child(even){background:#f8fafc!important;}.linha-orcamento:hover{background:#f1f5f9!important;}
      .status{display:inline-flex!important;align-items:center!important;justify-content:center!important;border:1px solid #cbd5e1!important;border-radius:999px!important;padding:3px 8px!important;background:#f8fafc!important;color:#111827!important;font-size:10px!important;font-weight:900!important;}
      .status-aprovado{background:#ecfdf5!important;color:#166534!important;border-color:#bbf7d0!important;}.status-recusado{background:#fff5f5!important;color:#b91c1c!important;border-color:#fecaca!important;}.status-finalizado{background:#eff6ff!important;color:#1d4ed8!important;border-color:#bfdbfe!important;}.status-em_servico{background:#f8fafc!important;color:#334155!important;border-color:#cbd5e1!important;}
      .msg-vazia,.msg-carregando,.msg-erro{padding:18px 14px!important;color:#64748b!important;font-size:13px!important;font-weight:800!important;text-align:center!important;}.msg-erro{color:#b91c1c!important;background:#fff5f5!important;}
      .fs-orc-modal-overlay{display:none;position:fixed;inset:0;z-index:59000;align-items:flex-start;justify-content:center;padding:16px;background:rgba(15,23,42,.66);overflow-y:auto;}.fs-orc-modal-overlay.ativo{display:flex;}
      .fs-orc-modal-box{width:min(860px,100%);margin-top:16px;background:#fff;border:1px solid #d1d5db;border-radius:12px;box-shadow:0 18px 42px rgba(15,23,42,.28);overflow:hidden;}
      .fs-orc-modal-header{display:flex;justify-content:space-between;align-items:flex-start;gap:10px;padding:12px 14px;background:#1f2937;border-bottom:1px solid #374151;color:#fff;}.fs-orc-modal-header h2{margin:0;color:#fff;font-size:18px;}.fs-orc-modal-header small{color:#e5e7eb;}
      .fs-orc-modal-body{padding:13px;color:#111827;}.fs-orc-modal-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-bottom:10px;}.fs-orc-info-card{border:1px solid #e5e7eb;background:#fff;border-radius:10px;padding:9px;min-width:0;}.fs-orc-info-card strong{display:block;color:#64748b;font-size:10px;text-transform:uppercase;margin-bottom:4px;}.fs-orc-info-card span{color:#111827;font-size:13px;font-weight:800;overflow-wrap:anywhere;}
      .botoes-modal{display:flex!important;flex-wrap:wrap!important;gap:6px!important;margin-top:12px!important;padding-top:10px!important;border-top:1px solid #e5e7eb!important;}.botoes-modal button,.botoes-modal a{min-height:34px!important;border-radius:8px!important;padding:7px 10px!important;border:1px solid #cbd5e1!important;background:#fff!important;color:#111827!important;font-size:12px!important;font-weight:900!important;text-decoration:none!important;cursor:pointer!important;}
      .btn-fechar-modal{width:32px;height:32px;border-radius:8px;border:1px solid #dc2626;background:#fff;color:#991b1b;font-size:20px;font-weight:900;cursor:pointer;}
  ...
  function prepararHtmlExistente() {
    ...
    if (!$('filtro-status')?.dataset.fsOrcamentosEvento) {
      $('filtro-status')?.addEventListener('change', (e) => trocarAbaOrcamentos(e.target.value));
      if ($('filtro-status')) $('filtro-status').dataset.fsOrcamentosEvento = '1';
    }
    if (!$('filtro-periodo')?.dataset.fsOrcamentosEvento) {
      $('filtro-periodo')?.addEventListener('change', (e) => trocarPeriodoResumo(e.target.value));
      if ($('filtro-periodo')) $('filtro-periodo').dataset.fsOrcamentosEvento = '1';
    }
    if (!$('filtro-busca')?.dataset.fsOrcamentosEvento) {
      $('filtro-busca')?.addEventListener('input', filtrarTabelaLocal);
      if ($('filtro-busca')) $('filtro-busca').dataset.fsOrcamentosEvento = '1';
    }
    if (!$('btn-toggle-resumo')?.dataset.fsOrcamentosEvento) {
      $('btn-toggle-resumo')?.addEventListener('click', (event) => { event.stopPropagation(); toggleResumoFinanceiro(); });
      if ($('btn-toggle-resumo')) $('btn-toggle-resumo').dataset.fsOrcamentosEvento = '1';
    }
  }
  ...
  async function tentarConsultaOrcamentos(campo, userId, ordem) {
    let consulta = window._supabase.from('orcamentos').select('*').eq(campo, userId).limit(200);
    if (ordem) consulta = consulta.order(ordem, { ascending: false });
  ...
  function atualizarResumo() {
    const resumo = { pendente: [0, 0], aprovado: [0, 0], recusado: [0, 0], em_servico: [0, 0], finalizado: [0, 0] };
  ...
  function renderizarTabelaOrcamentos(lista) {
    ...
    const linhas = estado.filtrados.map((orcamento) => {
      ...
      return `<tr class="linha-orcamento" onclick="abrirModalVisualizar('${id}')" title="Abrir orçamento"><td><strong>${escapar(numeroLista(orcamento))}</strong><br><small>${escapar(dataOrcamento(orcamento))}</small></td><td>${escapar(clienteNome(orcamento))}<br><small>${escapar(assuntoOrcamento(orcamento))}</small></td><td><strong>${moeda(totalOrcamento(orcamento))}</strong></td><td><span class="status status-${escapar(status)}">${escapar(statusOrcamento(status))}</span></td></tr>`;
    }).join('');
  ...
  async function carregarOrcamentos() {
    ...
    try {
      ...
      if (listaEl) listaEl.innerHTML = '<div class="msg-erro">Não foi possível carregar os orçamentos. Verifique política SELECT e vínculo usuario_id/user_id.</div>';
  ...
  function abrirModalVisualizar(id) {
    ...
    if (body) {
      body.innerHTML = `<div class="fs-orc-modal-grid"><div class="fs-orc-info-card"><strong>Cliente</strong><span>${escapar(clienteNome(orcamento))}</span></div><div class="fs-orc-info-card"><strong>Telefone</strong><span>${escapar(clienteTelefone(orcamento) || '-')}</span></div><div class="fs-orc-info-card"><strong>Total</strong><span>${moeda(totalOrcamento(orcamento))}</span></div><div class="fs-orc-info-card"><strong>Status</strong><span>${escapar(statusOrcamento(orcamento.status))}</span></div><div class="fs-orc-info-card"><strong>Data</strong><span>${escapar(dataOrcamento(orcamento))}</span></div><div class="fs-orc-info-card"><strong>Pagamento</strong><span>${escapar(orcamento.forma_pagamento_cliente || orcamento.forma_pagamento || orcamento.pagamento || '-')}</span></div><div class="fs-orc-info-card" style="grid-column:1/-1"><strong>Link do cliente</strong><span>${escapar(link)}</span></div><div class="fs-orc-info-card" style="grid-column:1/-1"><strong>Assunto/observações</strong><span>${escapar(assuntoOrcamento(orcamento))}</span></div></div><div class="botoes-modal"><a class="btn-primario-modal" href="${escapar(link)}" target="_blank" rel="noopener">Abrir link</a><button type="button" class="btn-whatsapp-orcamento" onclick="enviarWhatsAppOrcamentoVisualizado()">Enviar WhatsApp</button><button type="button" id="btn-pdf-orcamento-visualizado" class="btn-pdf-orcamento" onclick="baixarPdfOrcamentoVisualizado()">Baixar PDF</button><button type="button" onclick="copiarLinkOrcamento('${escapar(orcamento.id || '')}')">Copiar link</button><a href="/gerador.html?orcamento=${encodeURIComponent(orcamento.id || '')}">Editar/usar como base</a></div>`;
    }
  ...
  function enviarWhatsAppOrcamentoVisualizado() {
    ...
    const url = telefoneFinal ? `https://wa.me/${telefoneFinal}?text=${encodeURIComponent(mensagem)}` : `https://wa.me/?text=${encodeURIComponent(mensagem)}`;
    window.open(url, '_blank', 'noopener');
  }

  async function copiarLinkOrcamento(ref) {
    const link = linkPublicoOrcamento(ref);
    try {
  ...
  function carregarHtml2Pdf() {
    return new Promise((resolve, reject) => {
      ...
      if ($('fs-html2pdf-orcamentos')) {
        const checar = setInterval(() => { if (typeof window.html2pdf === 'function') { clearInterval(checar); resolve(); } }, 120);
  ...
  function linhasItens(itens) {
    ...
    if (!lista.length) return '<tr><td colspan="4" style="padding:12px;text-align:center;color:#64748b;">Nenhum item detalhado salvo.</td></tr>';
    return lista.map((item, indice) => `<tr style="background:${indice % 2 === 0 ? '#fff' : '#f8fafc'};border-bottom:1px solid #e5e7eb;"><td style="padding:10px;font-size:12px;color:#111827;">${escapar(item.descricao)}</td><td style="padding:10px;text-align:center;font-size:12px;color:#111827;">${escapar(item.qtd)}</td><td style="padding:10px;font-size:12px;color:#111827;">${moeda(item.valor)}</td><td style="padding:10px;text-align:right;font-size:12px;font-weight:bold;color:#111827;">${moeda(item.subtotal)}</td></tr>`).join('');
  }

  function montarHtml(orcamento, empresa) {
    const corPrimaria = '#334155';
    const corDestaque = '#94a3b8';
    ...
    const numero = numeroOrcamento(orcamento);
    const logoHtml = empresa?.foto_url ? `<img src="${escapar(empresa.foto_url)}" crossorigin="anonymous" style="max-width:128px;max-height:58px;object-fit:contain;">` : `<strong style="font-size:22px;color:${corPrimaria};">FS</strong>`;

    return `<div style="width:794px;min-height:1123px;box-sizing:border-box;padding:30px;background:#fff;font-family:Arial,sans-serif;color:#111827;"><div style="margin:-30px -30px 18px -30px;padding:18px 24px;background:${corPrimaria};color:#fff;border-bottom:4px solid ${corDestaque};display:flex;align-items:center;justify-content:space-between;gap:18px;"><div style="display:flex;align-items:center;gap:14px;min-width:0;"><div style="width:82px;height:62px;border-radius:12px;background:#fff;display:flex;align-items:center;justify-content:center;overflow:hidden;flex-shrink:0;">${logoHtml}</div><div><h1 style="margin:0;font-size:24px;line-height:1.15;color:#fff;">${escapar(nomeEmpresa)}</h1><div style="font-size:11px;line-height:1.45;opacity:.92;">${empresa?.telefone_empresa ? `WhatsApp: ${escapar(empresa.telefone_empresa)}<br>` : ''}${empresa?.cnpj_empresa ? `CNPJ/CPF: ${escapar(empresa.cnpj_empresa)}` : ''}</div></div></div><div style="text-align:right;font-size:11px;line-height:1.45;white-space:nowrap;">Gerado em<br><b>${new Date().toLocaleDateString('pt-BR')}</b><br>Nº ${escapar(numero)}</div></div><div style="background:#f8fafc;border-left:6px solid ${corDestaque};border-radius:10px;padding:14px 16px;margin-bottom:18px;display:flex;justify-content:space-between;gap:18px;align-items:flex-start;"><div style="width:55%;"><h2 style="margin:0 0 8px;color:${corPrimaria};font-size:22px;line-height:1.15;">${escapar(titulo)}</h2><div style="font-size:11px;line-height:1.5;color:#374151;"><b>EMISSOR</b><br>${escapar(nomeEmpresa)}<br>${empresa?.telefone_empresa ? `WhatsApp: ${escapar(empresa.telefone_empresa)}<br>` : ''}${empresa?.endereco_empresa ? `Endereço: ${escapar(empresa.endereco_empresa)}<br>` : ''}${empresa?.cnpj_empresa ? `CNPJ/CPF: ${escapar(empresa.cnpj_empresa)}` : ''}</div></div><div style="width:45%;text-align:right;font-size:12px;line-height:1.5;color:#374151;"><b>CLIENTE</b><br>${escapar(clienteNome(orcamento))}<br>${escapar(clienteTelefone(orcamento))}<br><b>Status:</b> ${escapar(statusOrcamento(orcamento?.status || 'pendente'))}</div></div><table style="width:100%;border-collapse:collapse;margin-top:8px;"><thead><tr style="background:${corPrimaria};color:#fff;"><th style="padding:10px;text-align:left;font-size:12px;">Item</th><th style="padding:10px;text-align:center;font-size:12px;">Qtd</th><th style="padding:10px;text-align:left;font-size:12px;">Unit.</th><th style="padding:10px;text-align:right;font-size:12px;">Subtotal</th></tr></thead><tbody>${linhasItens(orcamento?.itens)}</tbody></table><div style="margin-top:22px;text-align:right;"><div style="display:inline-block;min-width:190px;background:#f8fafc;border:1px solid #cbd5e1;border-radius:10px;padding:14px;"><span style="font-size:10px;color:#64748b;font-weight:bold;">VALOR TOTAL</span><br><strong style="font-size:22px;color:${corPrimaria};">${moeda(totalOrcamento(orcamento))}</strong></div></div><div style="margin-top:44px;display:flex;justify-content:space-between;gap:30px;align-items:flex-end;"><div style="font-size:10px;color:#64748b;line-height:1.45;"><b>${escapar(nomeEmpresa)}</b><br>Orçamento Nº ${escapar(numero)}</div><div style="width:240px;text-align:center;font-size:10px;color:#64748b;"><div style="border-top:1px solid #94a3b8;padding-top:6px;">Assinatura / Aprovação</div></div></div></div>`;
  }

  async function baixarPdfOrcamentoVisualizado() {
    const orcamento = encontrarOrcamentoVisualizado();
    if (!orcamento) return alert('Orçamento não encontrado para gerar PDF. Abra um orçamento novamente.');
    try {
  ...
  window.montarLinkOrcamento = linkPublicoOrcamento;
