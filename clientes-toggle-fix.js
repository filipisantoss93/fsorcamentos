/* =========================================================
   ...
   Padroniza o cadastro de cliente em modal, mantendo a página limpa.
   ========================================================= */
(function () {
  'use strict';
  ...
  function configurarModalCliente() {
    ...
    if (!eventosConfigurados) {
      ...
      card.addEventListener('click', (event) => {
        if (event.target === card) fecharModalCliente();
      });
  ...
  function injetarEstilo() {
    ...
    style.textContent = `
      ...
      #card-form-cliente.fs-modal-form-aberto > .clientes-card-header p {
        margin: 4px 0 0 !important;
        color: var(--fs-texto-suave, #6d4c41) !important;
        font-size: 12px !important;
  ...
  function interceptarLimpeza() {
    ...
    window.limparFormularioCliente = function () {
      original();
      const botao = document.getElementById('btn-toggle-form-cliente');
      if (botao && !document.getElementById('card-form-cliente')?.classList.contains('fs-modal-form-aberto')) {
        botao.textContent = '+ Novo cliente';
      }
    };
  ...
  function iniciar() {
    ...
    interceptarEdicao();
    interceptarLimpeza();
    setTimeout(() => { configurarModalCliente(); interceptarEdicao(); interceptarLimpeza(); }, 250);
    setTimeout(() => { configurarModalCliente(); interceptarEdicao(); interceptarLimpeza(); }, 900);
    setTimeout(() => { configurarModalCliente(); interceptarEdicao(); interceptarLimpeza(); }, 1800);
  }
