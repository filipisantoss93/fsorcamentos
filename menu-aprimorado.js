(function(){
  ...
  function aplicarCarregando(){const h=document.querySelector('.main-header');if(h)h.setAttribute('aria-busy','true');['btn-header-entrar','btn-header-sair','btn-menu-mobile-entrar','btn-menu-mobile-sair','btn-notificacoes'].forEach(id=>setDisplay($(id),'none'))}
  function aplicarResumo(r){ultimoResumo=r;removerItensEfexDuplicados();const saudacao=$('usuario-saudacao'),plano=$('fs-menu-plano'),saldo=$('fs-menu-saldo'),cta=$('fs-menu-cta'),h=document.querySelector('.main-header'),notificacoes=$('btn-notificacoes');if(r.estado==='visitante'){if(saudacao)saudacao.textContent='Olá, Convidado';if(plano)plano.textContent='Acesse sua conta';if(saldo)saldo.textContent='Crie orçamentos gratuitamente';setDisplay($('btn-header-entrar'),'inline-flex');setDisplay($('btn-header-sair'),'none');setDisplay($('btn-menu-mobile-entrar'),'inline-flex');setDisplay($('btn-menu-mobile-sair'),'none');setDisplay(notificacoes,'none');if(cta)cta.style.display='none';configurarLinksPremium(false)}else if(r.estado==='autenticado'){if(saudacao)saudacao.textContent=`Olá, ${r.nome}`;if(plano)plano.textContent=labelNivel(r.nivel);if(saldo)saldo.textContent=r.saldo===null?'Saldo Efex indisponível':`${r.saldo} crédito${r.saldo===1?'':'s'} Efex`;setDisplay($('btn-header-entrar'),'none');setDisplay($('btn-header-sair'),'inline-flex');setDisplay($('btn-menu-mobile-entrar'),'none');setDisplay($('btn-menu-mobile-sair'),'inline-flex');setDisplay(notificacoes,'inline-flex');configurarLinksPremium(r.premium);if(cta)cta.style.display=''}
  ...
  window.fsAtualizarResumoHeader=atualizar;window.fsFecharMenu=fecharMenu;
})();