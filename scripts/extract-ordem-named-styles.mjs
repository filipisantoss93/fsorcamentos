...
const htmlPath = path.join(raiz, 'ordem.html');
const cssPath = path.join(raiz, 'ordem-overrides.css');

if (!fs.existsSync(htmlPath)) {
  console.error('Arquivo ordem.html não encontrado. Rode este comando na raiz do projeto.');
  process.exit(1);
}
...
const estilosParaExtrair = [
  {
    ...
    titulo: 'FS formal theme overrides migrado de ordem.html'
  },
  {
    id: 'fs-contrast-fix-final',
    titulo: 'FS contrast fix final migrado de ordem.html'
  },
  {
    id: 'fs-ordem-mobile-fix',
    titulo: 'FS ordem mobile fix migrado de ordem.html'
  }
];
...
const linkCss = '<link id="fs-ordem-overrides-css" rel="stylesheet" href="ordem-overrides.css" />';

if (!html.includes('ordem-overrides.css')) {
  const marcadorPreferido = '<link id="fs-ordem-inline-css" rel="stylesheet" href="ordem-inline.css" />';
...
console.log('Estilos nomeados extraídos para ordem-overrides.css com sucesso.');
