...
const cssPath = path.join(raiz, 'painel-inline.css');
...
if (inicio === -1) {
  console.log('Nenhum primeiro bloco <style> simples encontrado em painel.html. Nada para extrair.');
  process.exit(0);
}
...
const marcadoresObrigatorios = [
  ...
  '.painel-card',
  '.painel-plano-aviso',
  '.btn-gerar-orcamento-painel'
];

const faltando = marcadoresObrigatorios.filter(marcador => !css.includes(marcador));
if (faltando.length) {
  console.error('O primeiro bloco <style> não parece ser o bloco principal de painel.html. Operação cancelada.');
  console.error(`Marcadores ausentes: ${faltando.join(', ')}`);
  process.exit(1);
}

const cssFinal = `/* =========================================================\n   FS ORÇAMENTOS - painel-inline.css\n   CSS principal extraído de painel.html\n   ========================================================= */\n\n${css}\n`;

fs.writeFileSync(cssPath, cssFinal, 'utf8');

const link = '<link id="fs-painel-inline-css" rel="stylesheet" href="painel-inline.css">';
html = `${html.slice(0, inicio).trimEnd()}\n  ${link}\n\n${html.slice(fim + '</style>'.length).trimStart()}`;
html = html.replace(/\n{4,}/g, '\n\n\n');

fs.writeFileSync(htmlPath, html, 'utf8');

console.log('Primeiro bloco <style> de painel.html extraído para painel-inline.css com sucesso.');
