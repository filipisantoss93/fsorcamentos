...
async function loginComProvider(provider) {
  try {
    ...
    fsLimparDestinoAposLogin();
    const { error } = await _supabase.auth.signInWithOAuth({ provider: providerNormalizado, options: { redirectTo: `${window.location.origin}/index.html`, queryParams: { access_type: 'offline', prompt: 'select_account' } } });
