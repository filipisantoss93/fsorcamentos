(function(){
  ...
  function dataHora(valor){
    ...
    return Number.isNaN(d.getTime()) ? 'Não definida' : d.toLocaleString('pt-BR');
  }
  function obterOrcamento(id){ return (window.orcamentosCache || []).find(item => String(item.id) === String(id)); }
  ...
  async function renovarLink(id, diasRecebidos){
    ...
    const { data, error } = await window._supabase.rpc('fs_renovar_link_orcamento',{p_orcamento_id:id,p_dias:dias});
    if(error || !data?.sucesso) return alert(data?.mensagem || error?.message || 'Não foi possível gerar o novo link.');
    await window.carregarOrcamentosPremium?.();
  ...
  async function revogarLink(id){
    ...
    const { data, error } = await window._supabase.rpc('fs_revogar_link_orcamento',{p_orcamento_id:id});
  ...
  function instalar(){
    if(!window._supabase || !Array.isArray(window.orcamentosCache)) {
    ...
    const renderOriginal = window.renderizarTabelaOrcamentos;
    window.renderizarTabelaOrcamentos = function(){
      const lista = typeof window.filtrados === 'function' ? window.filtrados() : window.orcamentosCache;
      if(typeof window.renderResumo === 'function') window.renderResumo(lista);
      const box = document.getElementById('lista-orcamentos');
      if(!box) return renderOriginal?.();
      ...
      box.innerHTML=`<table class="table"><thead><tr><th>Data</th><th>Cliente</th><th>Título</th><th>Status</th><th>Link</th><th>Pagamento</th><th>Total</th><th>Ações</th></tr></thead><tbody>${lista.map(o=>{
        const link=linkSeguro(o), st=window.statusOrc(o), pago=window.orcamentoPago(o), aprovado=st==='aprovado', sl=statusLink(o);
        return `<tr><td>${window.dbr(o.criado_em||o.created_at)}</td><td>${window.h(o.cliente_nome||'-')}</td><td>${window.h(o.titulo||o.assunto||'-')}</td><td><span class="status ${window.h(st)}">${window.h(o.status||'pendente')}</span></td><td><span class="payment-badge ${sl.classe}">${sl.texto}</span><br><small>${escapar(dataHora(o.public_link_expira_em))}</small></td><td>${aprovado?(pago?'<span class="payment-badge pago">Pago</span>':'<span class="payment-badge pendente">Pagamento pendente</span>'):'-'}</td><td><strong>${window.m(o.total)}</strong></td><td><div class="acoes"><button class="btn" onclick="abrirModalVisualizar('${escapar(o.id)}')">Ver</button><button class="btn" onclick="editarOrcamento('${escapar(o.id)}')">Editar</button><button class="btn" onclick="duplicarOrcamento('${escapar(o.id)}')">Duplicar</button>${linkAtivo(o)?`<button class="btn ok" onclick="enviarWhatsapp('${escapar(o.id)}')">WhatsApp</button><button class="btn" onclick="copiar('${escapar(link)}')">Copiar link</button><button class="btn red" onclick="fsRevogarLinkOrcamento('${escapar(o.id)}')">Revogar</button>`:`<button class="btn gold" onclick="fsRenovarLinkOrcamento('${escapar(o.id)}',90)">Gerar link</button>`}${aprovado&&!pago?`<button class="btn gold" onclick="marcarComoPago('${escapar(o.id)}')">Marcar pago</button>`:aprovado&&pago?'<button class="btn ok" disabled>Já no caixa</button>':''}</div></td></tr>`;
      }).join('')}</tbody></table>`;
    };

    window.abrirModalVisualizar = function(id){
      const o=obterOrcamento(id); if(!o) return;
      const link=linkSeguro(o), st=window.statusOrc(o), pago=window.orcamentoPago(o), sl=statusLink(o);
    ...
    window.enviarWhatsapp = function(id){
      ...
      const tel=texto(o.cliente_whatsapp).replace(/\D/g,'');
      if(!tel) return alert('Este orçamento não tem WhatsApp do cliente.');
