...
function fsNotificacoesInit() {
  ...
  setTimeout(verificarLembretesOrcamentosPendentes24h, 1800);
}
  ...
  modal.innerHTML = `
    <div class="modal-notificacoes-card">
      <div class="modal-notificacoes-topo">
        <div><strong>Notificações</strong><span>Aprovações, comunidade, agenda, OS e avisos importantes.</span></div>
      ...
      <div class="notificacoes-acoes">
        <button type="button" onclick="marcarTodasNotificacoesComoLidas()">Marcar todas como lidas</button>
        <a href="/dashboard.html">Abrir Dashboard</a>
      </div>
    </div>`;
...
function renderizarListaNotificacoes() {
  ...
  lista.innerHTML = notificacoesCache.map(n => {
    ...
    return `
      <div class="notificacao-item ${classeTipo} ${classeLida}" onclick="abrirNotificacao('${escaparAtributoNotificacao(n.id)}')">
        <div class="notificacao-icone">${icone}</div>
        <div class="notificacao-conteudo">
          <strong>${escaparHtmlNotificacao(n.titulo || 'Notificação')}</strong>
          <p>${escaparHtmlNotificacao(n.mensagem || '')}</p>
          <small>${formatarDataNotificacao(n.criado_em)}</small>
        </div>
        ${link ? '<button type="button">Abrir</button>' : ''}
      </div>`;
  }).join('');
}
...
function mostrarToastNotificacao(n) {
  ...
  const tipo = normalizarTipoNotificacao(n.tipo);
  const cor = tipo.includes('aprovado') ? '#16a34a' : tipo.includes('recusado') ? '#dc2626' : tipo.includes('forum') ? '#7c3aed' : tipo.includes('pix') ? '#2563eb' : '#3e2723';
  const toast = document.createElement('div');
  toast.id = 'toast-notificacao-orcamento';
  toast.innerHTML = `
    <div style="display:flex;gap:12px;align-items:flex-start;">
      <div style="width:38px;height:38px;border-radius:50%;background:${cor};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;">${obterIconeNotificacao(tipo)}</div>
      <div style="flex:1;"><strong style="display:block;margin-bottom:4px;color:#3e2723;">${escaparHtmlNotificacao(n.titulo || 'Notificação')}</strong><span style="display:block;color:#5d4037;font-size:14px;line-height:1.35;">${escaparHtmlNotificacao(n.mensagem || '')}</span><button type="button" onclick="abrirNotificacao('${escaparAtributoNotificacao(n.id)}')" style="margin-top:10px;background:#3e2723;color:#ffc400;border:1px solid #ffc400;padding:7px 11px;border-radius:8px;font-weight:800;cursor:pointer;">Abrir</button></div>
      <button type="button" onclick="fecharToastNotificacao()" style="background:transparent;border:none;color:#3e2723;font-size:22px;font-weight:bold;cursor:pointer;line-height:1;">×</button>
    </div>`;
