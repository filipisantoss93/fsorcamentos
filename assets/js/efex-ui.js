(()=>{
  ...
  function clearAllAndRestart(event){
    ...
    location.replace('/efex.html?nova='+Date.now());
  }
  ...
  function injectEnhancements(){
    ...
    style.textContent=`
      .efex-reset-all{border-color:#ef5350!important;color:#fff!important;background:#b3261e!important;font-weight:900!important}
      .efex-reset-all:hover{background:#8f1d18!important}
      .efex-result-new-analysis{display:flex!important;width:100%!important;justify-content:center!important;margin-top:12px!important;min-height:48px!important}
    ...
    const resultActions=document.querySelector('#efex-diagnostico-card .efex-result-actions');
    if(resultActions&&!document.getElementById('efex-nova-analise-resultado')){
      const button=document.createElement('button');
      button.id='efex-nova-analise-resultado';
      button.type='button';
      button.className='efex-btn primary efex-result-new-analysis';
      button.textContent='➕ Iniciar nova análise';
      button.addEventListener('click',clearAllAndRestart);
  ...
  function setImage(image,src,immediate){
    ...
    const swap=()=>{image.src=src;image.dataset.state=current;image.classList.remove('is-changing')};
  ...
  function numericText(value){const match=String(value||'').match(/\d+/);return match?Number(match[0]):0}
  ...
  injectEnhancements();
  preload('recepcao');preload('aguardando');preload('analisando');
  character?.addEventListener('error',fallbackImage);
  setState('recepcao');updateCounter();updateCreditProgress();
  ...
  function setResultVisibility(){
    ...
    if(active){setState('concluido');injectEnhancements()}
  }
  if(result){new MutationObserver(setResultVisibility).observe(result,{attributes:true,attributeFilter:['class']});setResultVisibility()}

  if(progress){
    new MutationObserver(()=>{
      const active=progress.classList.contains('active');
      const message=progress.textContent.toLowerCase();
      if(active){processingKind=message.includes('rascunho')||message.includes('orçamento')?'orcamento':'analise';setState(processingKind==='orcamento'?'gerandoOrcamento':'analisando');return}
  ...
  [$('efex-custo-relativo'),$('efex-cota')].filter(Boolean).forEach(node=>new MutationObserver(updateCreditProgress).observe(node,{childList:true,subtree:true,characterData:true}));
  ...
  if(dropzone&&input){
    ...
    dropzone.addEventListener('drop',event=>{
      ...
      try{const transfer=new DataTransfer();files.slice(0,4).forEach(file=>transfer.items.add(file));input.files=transfer.files;input.dispatchEvent(new Event('change',{bubbles:true}))}
  ...
  function setFollowupOpen(open){
    ...
    if(followupOpen){followupOpen.textContent=open?'Cancelar pergunta complementar':'Fazer pergunta complementar · 1 crédito';followupOpen.setAttribute('aria-expanded',String(open))}
  ...
  document.querySelectorAll('[data-efex-question]').forEach(button=>button.addEventListener('click',()=>{setFollowupOpen(true);if(followupInput){followupInput.value=button.dataset.efexQuestion||'';followupInput.focus()}}));

  function createCreditModal(){
    const modal=document.createElement('div');
    modal.id='efex-credit-modal';modal.className='efex-credit-modal';modal.hidden=true;
    modal.innerHTML=`<div class="efex-credit-modal-backdrop" data-close-modal></div><section class="efex-credit-dialog" role="dialog" aria-modal="true" aria-labelledby="efex-credit-modal-title"><button type="button" class="efex-credit-modal-close" data-close-modal aria-label="Fechar">×</button><span class="efex-credit-modal-icon">◉</span><h2 id="efex-credit-modal-title">Confirmar pergunta complementar</h2><p>Esta mensagem fará uma nova chamada ao EfeX.</p><div class="efex-credit-summary"><span>Custo desta pergunta</span><strong>1 crédito</strong></div><p class="efex-credit-modal-note">Reúna todos os novos dados em uma única mensagem para evitar consumo desnecessário.</p><div class="efex-credit-modal-actions"><button type="button" class="efex-btn secondary" data-close-modal>Cancelar</button><button type="button" class="efex-btn primary" data-confirm-credit>Confirmar e enviar</button></div></section>`;
    document.body.append(modal);return modal;
  }

  const creditModal=createCreditModal();
  let pendingSend=false;
  function closeCreditModal(){creditModal.hidden=true;document.body.classList.remove('efex-modal-open');pendingSend=false;followupSend?.focus()}
  function openCreditModal(){creditModal.hidden=false;document.body.classList.add('efex-modal-open');setTimeout(()=>creditModal.querySelector('[data-confirm-credit]')?.focus(),30)}
  creditModal.querySelectorAll('[data-close-modal]').forEach(button=>button.addEventListener('click',closeCreditModal));
  creditModal.querySelector('[data-confirm-credit]')?.addEventListener('click',()=>{if(!pendingSend)return;creditModal.hidden=true;document.body.classList.remove('efex-modal-open');pendingSend=false;followupSend?.onclick?.call(followupSend,new MouseEvent('click',{bubbles:true}))});
  document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!creditModal.hidden)closeCreditModal()});
  followupSend?.addEventListener('click',event=>{const question=followupInput?.value.trim()||'';if(!question)return;event.preventDefault();event.stopImmediatePropagation();pendingSend=true;openCreditModal()},true);
  draftButton?.addEventListener('click',()=>{if(!draftButton.disabled)setState('gerandoOrcamento')});

  window.EfexUI={setState,states:Object.keys(STATES),clearAllAndRestart,markSent:()=>setState('enviado'),markApproved:()=>setState('aprovado'),markBudgetReady:()=>setState('orcamentoGerado')};
})();